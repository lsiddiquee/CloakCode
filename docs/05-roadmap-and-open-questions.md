...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q1  | Command IDs for **Steer with Message** / **Stop and Send** / **Add to Queue**?                                                                                                                                            | "Steer" is the ideal remote blocker-answer primitive.                                                                                                                                                                                                                                                              | **RESOLVED (2026-07-09).** `vscode.commands.executeCommand('workbench.action.chat.open', { query })` **submits** a message to the panel chat (verified in the copilot-chat source: `{ query }` alone sends; adding `isPartialQuery: true` only populates). This is the **question** answer channel (docs/02 §4.7) — but it needs the **extension host** (`vscode.commands`), so the actuator moves off the dev-server. v1 targets the active session; multi-session targeting is a refinement. **SHIPPED (2026-07-15; steer path revised 2026-07-24):** all three actions are wired as RPC ops gated by `SessionSummary.inTurn`, each a **composer-free** `chat.submit {inputValue, …}` — `session.steer` → `chat.submit {inputValue, acceptInputOptions:{queue:'steering'}}`; `session.stop` → `chat.cancel` (or `chat.submit {inputValue, acceptInputOptions:{cancelCurrentRequest:true}}` for stop-and-send); queue / force-send → `chat.submit {inputValue}`. The old `chat.open`+`steerWithMessage` read the shared composer — that composer-capture caveat is now **SOLVED** (Known issues below). Command mechanics table in docs/03 “Mid-turn flag”.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
`SessionSummary.inTurn` (docs/02 §4.28/§4.34, docs/03 “Mid-turn flag”): while `inTurn` the composer
shows **steer** (default, `session.steer` → `chat.submit {inputValue, acceptInputOptions:{queue:'steering'}}`,
folds into the running turn) **and queue** (`session.respond` → `chat.submit {inputValue}`, waits for the
whole step) side by side, plus **stop-and-send** (`session.stop {text}` → `chat.submit {inputValue,
acceptInputOptions:{cancelCurrentRequest:true}}`, one atomic “Stop and Send”) and a pure **stop**
(`session.stop {}` → `chat.cancel`). Every send-path op passes `inputValue`, so the shared composer is
**never read** (a local desktop draft is untouched — composer-capture SOLVED 2026-07-24, docs/02.1 M3c).
Queue also doubles as the escape hatch when `inTurn` lags — it queues or sends either way. All
`remote-operator`, command-only, window-local/focus-dependent (docs/04). None leaves an on-disk marker —
a steer reads as a plain `user.message`, a stop as a normal `turn_end` (LIVE-CONFIRMED).
_Follow-up (SHIPPED 2026-07-16; source revised 2026-07-24):_ `inTurn` now **also streams live** over
`session.subscribe` (`{kind:"turn", inTurn}`, emitted on transition) alongside the `sessions.list`
snapshot, so the composer flips steer/queue↔send the moment the turn opens or closes without waiting for
a list refresh. The follower (`SessionFollower`) now derives the flag from the **debug-log's** turn spans
(`computeInTurnFromDebugLog`), not the transcript's `computeInTurn` (docs/02 §4.34).
...
- **Encrypted gateway links (security hardening) — DESIGN FINALIZED 2026-07-20; SHIPPED 2026-07-23
  ...
  **Finalized design — optional product-owned native TLS + PWA-delivered pin:**
  ...
  3. **Server identity to the extension: a SHA-256 fingerprint pin, delivered out-of-band via the
     authenticated PWA.** The gateway never prints/logs a secret; instead the PWA — already behind the
     Dev Tunnel sign-in **and** operator TOTP — exposes a **“Connect an extension”** action
     (`gateway.connectInfo` op) that shows a single **pairing URL**,
     `wss://<host>:<wssPort>#fp=<fingerprint>`, to paste into `cloakcode.gatewayUrl`; the address and
     its pin therefore cannot drift apart, and the fragment is never transmitted. The bare pin is shown
     too for anyone who prefers `cloakcode.gatewayCertFingerprint` as a separate setting. No cert PEM
     is delivered — the extension fetches the certificate itself and the pin is the whole trust
     decision. The PWA is a _different, already-verified_ channel (tunnel TLS + operator auth), so this
     is legitimate out-of-band provisioning, not blind TOFU. **Console is the fallback** when no tunnel
     is up; a gateway a **real authority** already vouches for needs no pin at all.
  4. **The extension always verifies — two ways to earn trust (`gateway-tls.ts`).**
     - **A real authority vouches for it.** The cert chains to a root the machine already trusts (a
       public CA, or an org root deployed to the device). Configure nothing: `rejectUnauthorized:true`
       against the system trust store plus the standard hostname check.
     - **You vouch for it (self-signed, the zero-setup default).** The fingerprint is the anchor:
       `selfProvisionedPin` fetches the certificate over a direct `tls.connect`, accepts it **only** if
       it matches the pin, and connects with it as the `ca` — so `rejectUnauthorized` stays true, full
       chain validation runs on every connection, and the fingerprint is re-checked in
       `checkServerIdentity`. It also skips the hostname/SAN check a bare-IP / `host.docker.internal`
       gateway can't satisfy. Pinning the exact cert is as strong as a CA for a single known server
       (verified live; "Mechanism 2"). With no pin and no trusted chain we still fail closed.

     The PWA only _delivers_ the pin; delivery never substitutes for verification. Explicitly
     rejected: blind first-connection TOFU and learning the pin over the unverified socket. The
     certificate fetch does set `rejectUnauthorized:false` on a **throwaway** socket that carries no
     application data, and its result is accepted only on an exact fingerprint match — a pin, not TOFU.
  ...
  **Shipped surfaces (C1–C4):** gateway `startGateway` `tls` option + the second `wss://` listener
  ...
  (`ConnectExtensionView`); the pinned extension client (`gateway-tls.ts`) + the pairing-URL codec
  (`@cloakcode/protocol`) behind `cloakcode.gatewayUrl` / `cloakcode.gatewayCertFingerprint`
  (machine-scoped, S4a); env `CLOAKCODE_TLS_PORT` /
...
- **Steer composer-capture — RESOLVED (2026-07-24).** The earlier steer prefilled the **shared**
  ...
  been cleaned up / Terminal output:" wording) or a local half-typed message could merge into the
  single `remote-operator`-attributed `user.message` — a provenance break (docs/04 rule 4; verified
  live 2026-07-23). **Fix:** steer is now a composer-free `chat.submit {inputValue,
  acceptInputOptions:{queue:'steering'}}` — `SubmitAction.run` (`chatExecuteActions.ts`) forwards
  `inputValue` to `widget.acceptInput`, and `_acceptInput` uses OUR text (`isUserQuery = !query`), so
  the composer is **never read** — no capture window. The old "unreachable from an extension" verdict
  was **wrong**: the 2026-07-08 "Failed to find command" was the **string-only** `run_vscode_command`
  probe, which can't pass the OBJECT context `{ inputValue, acceptInputOptions }`; an **extension**
  `executeCommand` passes a real object over the ext-host RPC, and `chat.submit`'s `Action2`
  precondition gates only menus/keybindings, **not** `executeCommand`, so it runs mid-turn (VS Code's
  own "Stop and Send" does exactly `executeCommand(ChatSubmitAction.ID, { acceptInputOptions:
  { cancelCurrentRequest: true } })`). Full write-up: docs/02.1 M3c. Relates to the **B4** structural
  per-message provenance thread (docs/04).

- **Pure Stop leaves `inTurn` stuck true until the next turn — minor, PARKED (2026-07-24).** A pure
  **Stop** (`session.stop {}` → `chat.cancel`) writes **no** `turn_end` to the debug-log, so the
  follower's `inTurn` (now debug-log-sourced, docs/02 §4.34) stays true after a Stop until the next
  real turn opens/closes. A client-side **optimistic** reset (`ChatComposer` `onStopped` → dispatch
