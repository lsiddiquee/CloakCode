...
| `cloakcode.gatewayCaFile`     | _(empty)_ | **Optional (stricter).** Gateway TLS cert (PEM) for full-chain `wss://` validation of a **self-signed** gateway. Not required — the fingerprint below alone pins it; add this only for stricter validation. Not needed for a real/BYO CA. |
| `cloakcode.gatewayCertFingerprint` | _(empty)_ | Expected **SHA-256 fingerprint** of the gateway's TLS cert — the out-of-band pin. For a self-signed gateway this is **all you need**: the exact cert is verified on every `wss://` connect and a mismatch **fails closed**. Copy it from the gateway's **Connect an extension** view. |
