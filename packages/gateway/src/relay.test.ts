...
describe("Relay", () => {
  ...
  it("only lets the routed-to provider answer a relay (F13)", () => {
    const relay = new Relay();
    const operator = fakeOperator();
    const providerA = {};
    const providerB = {};
    ...
    relay.forward(
      asWs(operator),
      { id: "op-1", op: "session.subscribe", params: {} },
      providerA,
      (text) => {
        relayId = (JSON.parse(text) as { id: string }).id;
      },
    );
    // A frame carrying this relay id but from a DIFFERENT provider is swallowed.
    expect(
      relay.routeProviderFrame(JSON.stringify({ id: relayId }), providerB),
    ).toBe(true);
    expect(operator.sent).toHaveLength(0);
    // The provider the relay was routed to IS allowed to answer.
    expect(
      relay.routeProviderFrame(JSON.stringify({ id: relayId }), providerA),
    ).toBe(true);
