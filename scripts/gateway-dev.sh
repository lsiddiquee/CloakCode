...
Options:
  ...
  --bind <address>     Gateway bind address (default: 127.0.0.1)
  --port <number>      Gateway port (default: 7900; 0 selects an ephemeral port)
...
The equivalent CLOAKCODE_GATEWAY_HOST, CLOAKCODE_GATEWAY_PORT,
CLOAKCODE_TUNNEL, CLOAKCODE_MFA, and CLOAKCODE_INSTANCE_ID environment
variables remain supported. Explicit switches take precedence.
